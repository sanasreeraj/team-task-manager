...
import { LayoutDashboard, FolderKanban, CheckSquare, Users, ClipboardList, LogOut, Settings } from 'lucide-react'
...
export function Sidebar({ logoutAction, role, userName, isAdmin }: { logoutAction: () => void, role: string, userName: string, isAdmin: boolean }) {
  ...
  return (
    <>
      ...
      <div className="hidden md:flex h-full w-64 flex-col bg-card border-r border-border/60 transition-colors duration-300">
        {/* Logo */}
      ...
      {/* Mobile Top Bar */}
      <div className="md:hidden flex h-14 shrink-0 items-center justify-between px-4 border-b border-border/40 bg-card/80 backdrop-blur-md sticky top-0 z-40">
        <h1 className="text-lg font-bold text-foreground tracking-tight">Task Manager</h1>
        <Link href="/dashboard/settings" className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-xs font-bold text-primary uppercase">
          {userName?.charAt(0) || 'U'}
        </Link>
      </div>

      {/* Mobile Bottom Navigation */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-card border-t border-border/40 pb-safe">
        <div className="flex items-center justify-around h-16 px-2">
          {navigation.map((item) => {
            ...
            return (
              ...
              >
                <item.icon className={cn('w-5 h-5', active && 'fill-primary/20')} aria-hidden="true" />
